...
import { Music2, Eye, EyeOff, Loader, ArrowRight, Lock } from 'lucide-react';
...
export default function ResetPassword() {
  ...
  const { setUser } = useAuthStore(); // Actually we want to just navigate to login or set token
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('tunify_token', res.data.token);
    ...
    <div className="min-h-screen flex poster-grid" style={{ background: '#080808' }}>
      ...
      <div className="hidden lg:flex flex-1 relative overflow-hidden items-end p-12">
        ...
        <div className="relative z-10">
          ...
          <h1 className="font-black leading-[0.9] tracking-tighter text-white mb-2" style={{ fontSize: '5rem' }}>NEW</h1>
          ...
          <p className="text-white/40 text-base max-w-xs leading-relaxed">
            Set a new password to secure your account and get back to the music.
          </p>
        ...
        <div className="absolute top-10 left-10 text-xs font-bold text-white/20 uppercase tracking-[0.3em]">04*</div>
      </div>
      ...
      <div className="w-full lg:w-[420px] flex-shrink-0 flex items-center justify-center p-8">
        ...
        >
          ...
          <h2 className="text-3xl font-black text-white mb-1">Set new password</h2>
          <p className="text-white/30 text-sm mb-8">Enter your new password below</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-xl font-bold text-white flex items-center justify-center gap-2 mt-4 transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
              style={{ background: 'linear-gradient(135deg,#F97316,#EA580C)', boxShadow: '0 0 25px rgba(249,115,22,0.5)' }}
            >
              {loading ? <Loader size={18} className="animate-spin" /> : <>Update Password <ArrowRight size={16} /></>}
            </button>
          </form>
